...
class VanillaMLP(nn.Module):
    def __init__(self, n_inputs=8, n_hidden=16, n_classes=2):
        super(VanillaMLP, self).__init__()
        # replaces he_init() and global variables
        # PyTorch holds W1, b1, W2, and b2 safely inside this object
        self.layer1 = nn.Linear(n_inputs, n_hidden)
        self.relu = nn.ReLU()
        self.layer2 = nn.Linear(n_hidden, n_classes)
    
    def forward(self, x):
        # this is the forward function from before
        Z1 = self.layer1(x)
        A1 = self.relu(Z1)
        Z2 = self.layer2(A1)
        return Z2 # return raw scores, and CrossEntropyLoss handles softmax()
    
# instantiating tools
model = VanillaMLP()

# replace cross_entropy_loss() function
criterion = nn.CrossEntropyLoss()

# replaces update_weights() (handles lr natively)
optimizer = optim.SGD(model.parameters(), lr=0.01)

# replaces accuracy() function
...
for epoch in range(epochs):
    ...
    # backward pass (replaces backward() function)
    optimizer.zero_grad()
    loss.backward()

    # update weights (replaces update_weights() function)
    optimizer.step()
